<template>
  <div
    class="fixed pt-6 bottom-0 px-20 z-50 h-48 shadow-custom-up-heavy bg-white border-t border-gray-200 dark:bg-gray-700 dark:border-gray-600"
    ...
    <div class="w-full h-full">
      <div class="flex gap-16">
        ...
        <div class="flex-col">
          ...
          <div class="mt-1">
            Jl. KH AGUS SALIN GG II NO. 9, Desa/Kelurahan Songgokerto, Kec. Batu, Kota Batu, Provinsi Jawa Timur
          </div>
        </div>
      </div>
      <hr class="h-[0.125rem] text-gray-300 bg-gray-300 my-4" />
      <div class="flex justify-between align-items-center">
        ...
        <div>
          ...
          <button class="px-14 py-2 bg-primaryColor rounded-[5px] text-white"
            @click="submitPesanan(selectedMethod)">Bayar</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const selectMethod = (method) => {
      ...
      emit('update-method', method); // Emit event dengan metode pembayaran yang dipilih
    };
